...
export default function Detail({ record }) {
  ...
  const shareText = encodeURIComponent(
    `${f.street_name || 'Propiedad'} – $${(f.price_current||0).toLocaleString()}\n` +
    `${SITE_URL}/${record.id}`
  );
    ...
    <>
      ...
      <main>
        ...
        <section className="features">
          ...
          {f.interior_features && <p><strong>Interior:</strong> {f.interior_features}</p>}
          {f.exterior_features && <p><strong>Exterior:</strong> {f.exterior_features}</p>}
